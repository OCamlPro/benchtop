...
let retrieve ~db_file ?db_attached req =
  let prefix = Filename.concat Options.db_dir in
  let db_file_path = prefix db_file in
  let db_attached_path = Option.map prefix db_attached in
  let db_uri = Format.sprintf "sqlite3://%s" db_file_path |> Uri.of_string in
...
module Res = struct
  type t = Sat | Unsat | Unknown | Error | Timeout | Unexpected of string
...
module Errcode = struct
  ...
  let decode = function 0 -> Ok Success | i -> Ok (Failed i)
...
module Prover = struct
  ...
  let select =
    [%rapper
      get_many
        "SELECT @string{name}, @string{version} FROM prover WHERE (name = \
         %string?{name} OR %string?{name} IS NULL) AND (version = \
         %string?{version} OR %string?{version} IS NULL)"
        record_out]
  ...
  let select_one =
    [%rapper
      get_one
        "SELECT @string{name}, @string{version} FROM prover WHERE name = \
         %string{name} AND (version = %string?{version} OR %string?{version} \
         IS NULL)"
        record_out]
...
module Problem = struct
  ...
  let count ?(file = "") ~res ~file_expect ~errcode ~only_diff
    ...
    Db.find
      ...
      @@ Format.asprintf
           "SELECT COUNT(*)\n\
           \        FROM prover_res WHERE file LIKE '%%%s%%' AND (res IN (%a) \
            OR '%a' = '') AND (file_expect IN (%a) OR '%a' = '') AND (errcode \
            IN (%a) OR '%a' = '') AND (NOT %B OR (res <> file_expect))"
           file
           (Misc.pp_list (pp_quote Res.pp))
           res (Misc.pp_list Res.pp) res
           (Misc.pp_list (pp_quote Res.pp))
           file_expect (Misc.pp_list Res.pp) file_expect
           (Misc.pp_list (pp_quote Errcode.pp))
           errcode (Misc.pp_list Errcode.pp) errcode only_diff)
      ()

  let select, select_one =
    let function_out
        ( file,
          (res, (file_expect, (timeout, (stdout, (stderr, (errcode, rtime))))))
        ) =
      { file; res; file_expect; timeout; stdout; stderr; errcode; rtime }
    in
    let open Caqti_type.Std in
    let open Caqti_request.Infix in
    let output =
      Caqti_type.(
        tup2 string
          (tup2 Res.t
             (tup2 Res.t
                (tup2 int (tup2 octets (tup2 octets (tup2 Errcode.t float)))))))
    in
    ( (fun ?(file = "") ~res ~file_expect ~errcode ~only_diff ~page
        ...
        Db.collect_list
          ((unit ->* output)
          @@ Format.asprintf
               "SELECT file, res, file_expect, timeout, stdout, stderr, \
                errcode, rtime FROM prover_res WHERE file LIKE '%%%s%%' AND \
                (res IN (%a) OR '%a' = '') AND (file_expect IN (%a) OR '%a' = \
                '') AND (errcode IN (%a) OR '%a' = '') AND (NOT %B OR (res <> \
                file_expect)) LIMIT 50 OFFSET (50*%i)"
               file
               (Misc.pp_list (pp_quote Res.pp))
               res (Misc.pp_list Res.pp) res
               (Misc.pp_list (pp_quote Res.pp))
               file_expect (Misc.pp_list Res.pp) file_expect
               (Misc.pp_list (pp_quote Errcode.pp))
               errcode (Misc.pp_list Errcode.pp) errcode only_diff page)
          ()
        >|? List.map function_out),
      fun ?(name = "NULL") (module Db : Caqti_lwt.CONNECTION) ->
        Db.find
          ((unit ->! output)
          @@ Format.asprintf
               "SELECT file, res, file_expect, timeout, stdout, stderr, \
                errcode, rtime FROM prover_res WHERE file LIKE '%%%s%%'"
               name)
          ()
        >|? function_out )
end
...
module Round_summary = struct
  ...
  let retrieve =
    [%rapper
      get_one
        "SELECT CAST ((SELECT value FROM meta WHERE key = 'uuid') AS TEXT) AS \
         @string{uuid},CAST ((SELECT value FROM meta WHERE key = 'timestamp') \
         AS REAL) AS @Time{running_at},(SELECT COUNT(file) FROM prover_res) AS \
         @int{ctr_pbs},(SELECT COUNT(file) FROM prover_res WHERE res = \
         file_expect) AS @int{ctr_suc_pbs}"
        record_out]
end
...
module Problem_diff = struct
  ...
  let count =
    [%rapper
      get_one
        ...
        \        JOIN other.prover_res as p2 ON p1.file = p2.file WHERE \
         ...
         ROUND(p1.rtime-p2.rtime, 0) < -1 AND NOT (p1.res = 'timeout' AND \
         ...
         <> p1.file_expect) OR (((p1.rtime-p2.rtime)/p1.rtime > \
         %float{threshold} AND ROUND(p1.rtime-p2.rtime, 0) > 1 AND NOT (p1.res \
         ...
         ABS(ROUND(p1.rtime-p2.rtime, 0)) > 1 AND NOT (p1.res = 'timeout' AND \
  ...
  let select =
    [%rapper
      get_many
        "SELECT p1.file AS @string{file1}, p1.res AS @Res{res1}, \
         p1.file_expect @Res{file_expect1}, p1.timeout @int{timeout1}, \
         p1.stdout AS @octets{stdout1}, p1.stderr AS @octets{stderr1}, \
         p1.errcode AS @Errcode{errcode1}, p1.rtime AS @float{rtime1}, p2.file \
         AS @string{file2}, p2.res AS @Res{res2}, p2.file_expect \
         @Res{file_expect2}, p2.timeout @int{timeout2}, p2.stdout AS \
         @octets{stdout2}, p2.stderr AS @octets{stderr2}, p2.errcode AS \
         @Errcode{errcode2}, p2.rtime AS @float{rtime2} FROM main.prover_res \
         ...
         <> p2.file_expect) OR (((p1.rtime-p2.rtime)/p1.rtime < \
         -%float{threshold} AND ROUND(p1.rtime-p2.rtime, 0) < -1 AND NOT \
         ...
         %Kind_diff{kind_diff} = 'difference'))\n\
        \      ORDER BY CASE WHEN\n\
        \        p1.res <> p2.res OR p1.file_expect <> p2.file_expect OR \
         p1.errcode <> p2.errcode THEN 0\n\
        \        ELSE 1 END\n\
        \      LIMIT 50 OFFSET (50 * %int{page})" function_out]
      (fun
        ~file1
        ~res1
        ~file_expect1
