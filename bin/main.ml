...
let header_logger inner_handler request =
  ...
  let pp_fields =
    Misc.pp_list (fun fmt (key, value) -> Format.fprintf fmt "%s: %s" key value)
  in
  ...
  let _ =
    ...
    | Some content_type -> (
        ...
        | "application/x-www-form-urlencoded" :: _ -> (
            ...
            | `Ok form_fields ->
                Dream.debug (fun log ->
                    log "POST fields:@,%a" pp_fields form_fields)
            | _ -> ())
        | _ -> Lwt.return ())
    | None -> Lwt.return ()
  in
...
module Cmd = struct
  ...
  let log_level =
    ...
    in
    Arg.(value & opt level `Info & info [ "l"; "log" ] ~docv:"LOG_LEVEL" ~doc)
  ...
  let cmd =
    ...
    Cmd.v info
      Term.(
        const main $ log_level $ interface $ port $ number_of_jobs $ share_dir)
